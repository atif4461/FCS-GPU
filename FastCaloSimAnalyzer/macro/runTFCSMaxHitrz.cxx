 ...
 * @brief Creates a chain to read 2D alpha radius in mm from the input file.
 
TFCSParametrizationChain RunInputHits(std::string name, std::string title)
 * @brief Initializes the TH2 histogram for hit energy vs alpha radius.
 *
 * @param prefix String prefix for the histogram name.
 * @return Pointer to the initialized TH2 histogram.
 
TH2* InitTH2(std::string prefix, std::string title, int nbinsx, float xmin, float xmax, int nbinsy, float ymin, float ymax)
 * @brief Creates a 2D histogram from the original hit energy vs alpha radius histogram.
 *
 * @param h_original Original TH2 histogram.
 * @param energy_cutoff Energy cutoff value.
 * @return Pointer to the created TH2 histogram.
 
TH2F* Create2DHistogram(TH2* h_original, float energy_cutoff)
 * @brief Saves the polar plot of the 2D histogram.
 *
 * @param h_alpha_r Pointer to the TH2 histogram.
 * @param plotDir Directory path for saving the plot.
 
void CreatePolarPlot(TH2F* h_alpha_r, std::string plotDir)
 * @brief Loops over events for the specified PCA bin.
 *
 * @param pcabin PCA bin number.
 
void LoopEvents(int pcabin)* This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
